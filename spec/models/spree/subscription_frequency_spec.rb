...
RSpec.describe Spree::SubscriptionFrequency, type: :model do
  ...
  describe "validations" do
    ...
    it { is_expected.to validate_numericality_of(:days_count).is_greater_than(0).only_integer }
    context "uniqueness of title" do
      let!(:subscription_frequency_1) { create(:daily_subscription_frequency) }
      let(:subscription_frequency_2) { build(:daily_subscription_frequency) }
